...
const Edit_chat_template = () => {
    ...
    useEffect(() => {
        ...
        axios({
            method: 'POST',
            url: `${process.env.REACT_APP_BASE_URL}/fetch/template/${id}`,
            headers: {
                'authorization': `Bearer ${token}`
            }
        })
            .then((res) => {
                ...
                setEmailTitle(res.data.data.msg_title)
                setEmailType(res.data.data.email_type)
                setTemplatedata(res.data.data.msg_content)
                setEditorContent(res.data.data.msg_content)
...
export default Edit_chat_template;
