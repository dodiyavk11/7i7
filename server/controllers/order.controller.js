...
exports.GetFilteredOrder = async (req, res) => {
  try {
    ...
    const fetchOrderByUIddd = filterComboArr.map((val, i) => {
      return fetchOrderByUIds.filter((innerVal, i) => {
        return (
          innerVal.orderstatus === val[0] && innerVal.orderpriority === val[1]
        );
    ...
    fetchOrderByUIddd.map((val) => {
      val.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
    });

    const allDoneOrders = fetchOrderByUIddd.map((val) => {
      const DoneOrders = val.filter((innerVal) => innerVal.orderstatus === 3);
      const WithoutDoneOrders = val.filter((innerVal) => innerVal.orderstatus !== 3);
      return WithoutDoneOrders.concat(DoneOrders.reverse())
  })

    const fetchOrderByUId=allDoneOrders.flat(Infinity);
...
exports.FileAdd = async (req, res) => {
  try {
    ...
    // console.log(JSON.stringify(cloud_links));
    // console.log(req.body, "bodybodybodybodybodybodybodybodybody")

    const order = orderfile?.length > 0 && orderfile.map((val, i) => {
...
//! file add cloud links
exports.AddCloudLinkFinalFile = async (req, res) => {
  try {
    const  orderid = req.params.id;
    const {cloud_links} = req.body;
    // console.log(cloud_links, cloud_links.length)
    
    if(!cloud_links || cloud_links.length === 0) return res.status(404).send({status: false,message: "Bitte Link hinzufügen",data:[]});
    
    const filteredLink = cloud_links.filter((val) => {
        // console.log(val.link)
        if(val.link) return val
    })
    
    
        // console.log(filteredLink, "ddddddddddddddddddddddd")
    const CLinks = await Promise.all(
    await Promise.all(filteredLink.map(async val =>{
     await Models.file_media.create({
        orderid,
        isLink:true,
        link:val.link,
    ...
    res
      ...
      .send({
        status: true,
        message: "Link erfolgreich hinzugefügtt",
        data:CLinks
      });
  } catch (err) {
    console.log(err);
    res
      ...
      .send({
        status: false,
        message:
          "Der Link kann nicht hinzugefügt werden, da ein Fehler aufgetreten ist",
        data: [],
        error: err.message,
      });
  }
};
exports.UpdateCloudLinkFinalFile = async (req, res) => {
  try {
    const  orderid = req.params.id;
    const {cloud_links} = req.body;
    // console.log(cloud_links, cloud_links.length);
    ...
    const CLinks = await Promise.all(
     ...
     await Models.file_media.update({
        link:cloud_links,
        },{ where: { id: orderid } })
    );
...
//! delete cloud links final file
exports.DeleteCloudLinkFinalFile = async (req, res) => {
  try {
    const { id } = req.params;
    const removeOrder = await Models.file_media.destroy({ where: { id } });
    res
...
exports.UpdateOrderStatus = async (req, res) => {
  try {
    ...
    if (orderstatus == 1) {
      text = text.replace("{order_status}", "Neuer Auftrag");
      const mail = await emailTemplate(text);
